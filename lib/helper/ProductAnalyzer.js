...
exports.ProductAnalyzer = void 0;
/**
...
class ProductAnalyzer {
     ...
     */
    static getExtremePointsInIntervalWithTime(csv, start, end, type) {
     ...
     */
    static getValueAtTime(csv, time, type) {
        if (!csv || csv.length === 0) {
            return -1;
        }
        let i = 0;
     ...
     */
    static getPriceAndShippingAtTime(csv, time) {
        if (!csv || csv.length === 0) {
            return [-1, -1];
        }
        let i = 0;
     ...
     */
    static getLastPriceAndShipping(csv) {
        if (!csv || csv.length < 3) {
            return [-1, -1];
        }
     ...
     *          If the CSV includes shipping costs, the result will be the total landing price (price + shipping).
     */
    static getClosestValueAtTime(csv, time, type) {
        if (!csv || csv.length === 0) {
            return -1;
        }
        let i = 0;
     ...
     */
    static getLowestAndHighest(csv, type) {
        const minMax = this.getExtremePointsInIntervalWithTime(csv, 0, Number.MAX_VALUE, type);
     ...
     */
    static getLowestAndHighestWithTime(csv, type) {
        return this.getExtremePointsInIntervalWithTime(csv, 0, Number.MAX_VALUE, type);
    }
    /**
     ...
     */
    static calcWeightedMean(csv, now, days, type) {
        return this.getWeightedMeanInInterval(csv, now, now - Math.floor(days * 24 * 60), now, type);
    }
    /**
     ...
     */
    static getWeightedMeanInInterval(csv, now, start, end, type) {
        let avg = -1;
        ...
        for (let i = 1; i < size; i += loopIncrement) {
            ...
            if (start <= date) {
                ...
                let nextDate = i + adjustedIndex === size ? now : csv[i + adjustedIndex];
                if (end < nextDate) {
                    nextDate = end;
                }
     ...
     */
    static getOutOfStockInInterval(csv, start, end, type) {
        if (type.isWithShipping) {
            if (!csv || csv.length < 6) {
                return null;
            }
     ...
     */
    static getOutOfStockPercentageInInterval(csv, now, start, end, type, trackingSince) {
        if (!type.isPrice) {
            return -1;
        }
        if (end <= start) {
...
exports.ProductAnalyzer = ProductAnalyzer;
